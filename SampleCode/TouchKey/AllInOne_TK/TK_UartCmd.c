...
#define _TXBUFSIZE   E_TX_CMD_TYPE2_SIZE
#define _RXBUFSIZE   E_RX_CMD_TYPE1_SIZE
#define E_UART_TIME_OUT     (0x200000)
uint8_t gu8TXBuf[_TXBUFSIZE];
uint8_t gu8RXBuf[_RXBUFSIZE];


#ifdef OPT_NEIGHBOR
//S_NEIGHBOR s_neighbor[/*TKLIB_TOL_NUM_KEY*/ u8MaxScKeyNum];
#endif
...
#endif
//    printf("CMD1 = 0x%x\n", *pu8RXBuf);
//      printf("A POLSEL=0x%x\n", TK->POLSEL);
    switch (*pu8RXBuf)
    {
    case E_CMD_TYPE1_SPECIFY_CHANNEL_MSK:
        psTkInfo->u32EnChanMsk = pu8RXBuf[1] | ((uint32_t)(pu8RXBuf[2]) << 8) | ((uint32_t)(pu8RXBuf[3]) << 16) | ((uint32_t)(pu8RXBuf[4]) << 24);//u32Data;
        break;


    case E_CMD_TYPE1_SPECIFY_SLIDER_MSK:
        psTkInfo->u32EnSliderMsk = pu8RXBuf[1] | ((uint32_t)(pu8RXBuf[2]) << 8) | ((uint32_t)(pu8RXBuf[3]) << 16) | ((uint32_t)(pu8RXBuf[4]) << 24);//u32Data;
        //printf("Slider Channel Msk L= 0x%x\n", (uint16_t)(psTkInfo->u32EnSliderMsk&0xFFFF));
        //printf("Slider Channel Msk H= 0x%x\n", (uint16_t)(psTkInfo->u32EnSliderMsk>>16));
        break;

    case E_CMD_TYPE1_SPECIFY_WHEEL_MSK:
        psTkInfo->u32EnWheelMsk = pu8RXBuf[1] | ((uint32_t)(pu8RXBuf[2]) << 8) | ((uint32_t)(pu8RXBuf[3]) << 16) | ((uint32_t)(pu8RXBuf[4]) << 24);//u32Data;
        //printf("Wheel Channel Msk L= 0x%x\n", (uint16_t)(psTkInfo->u32EnWheelMsk&0xFFFF));
        //printf("Wheel Channel Msk H= 0x%x\n", (uint16_t)(psTkInfo->u32EnWheelMsk>>16));
        break;
...
//        psTkInfo->u8PolarityCapacitorBank = 2; //(20200807 removed)//pu8RXBuf[3];    /* Specified the level for enabled channels on non-measured period */
        psTkInfo->u8PolarityCapacitorBank = pu8RXBuf[3];
        break;
...
//        psTkInfo->u8AvcchLevel = 7; //(20200807 removed) //pu8RXBuf[3];
    ...
    case E_CMD_TYPE1_SPECIFY_STORE_ADDR:
        psTkInfo->u32StoreAddr = pu8RXBuf[1] | ((uint32_t)(pu8RXBuf[2]) << 8) | ((uint32_t)(pu8RXBuf[3]) << 16) | ((uint32_t)(pu8RXBuf[4]) << 24);//u32Data;
        break;
...
//                      psTkFeat->u32PinSel1 = 0x00000000;
    ...
    case E_CMD_TYPE1_START_RUN_TIME_SCAN_KEY:
        ...
        {
            uint8_t chan;
            for(chan = 0; chan < /*TKLIB_TOL_NUM_KEY*/ u8MaxScKeyNum; chan = chan+1)
            {
    ...
    //UART_WAIT_TX_EMPTY(TK_UART_PORT);  /* Unnecessary due TK_SendChars() will wait until complete */
    return i8Ret;
}
...
//      printf("A POLSEL=0x%x\n", TK->POLSEL);
    ...
    case E_CMD_TYPE2_FIRMWARE_VERSION:/* 2020-0324 */
        ...
        else
        {
            //for PDID
            u32PDID = SYS->PDID;
            gu8TXBuf[0] = (uint8_t)((u32PDID & 0xFF000000)>>(24));
            gu8TXBuf[1] = (uint8_t)((u32PDID & 0x00FF0000)>>(16));
            gu8TXBuf[2] = (uint8_t)((u32PDID & 0x0000FF00)>>(8));
            gu8TXBuf[3] = (uint8_t)((u32PDID & 0x000000FF));
        }
        break;
#if 0
    case E_CMD_TYPE2_REPORT_PRESSED_TK_CHANNELS:
        gu8TXBuf[0] = 0xFF;
        gu8TXBuf[1] = 0xFF;
        gu8TXBuf[2] = 0xFF;
        gu8TXBuf[3] = 0xFF;
...
//    uint8_t u8checkSum = 0;
    ...
    do
    {
        ...
        {
#if 0
            while (UART_IS_TX_FULL(TK_UART_PORT)); /* Wait Tx is not full to transmit data */
            UART_WRITE(TK_UART_PORT, 'N');
#else
            gu8TXBuf[0] = 'N';
            TK_SendChars(gu8TXBuf, 1);
#endif
...
//    if ((gu8RXBuf[0] >= (uint8_t)0x0) && (gu8RXBuf[0] <= E_CMD_TYPE1_NUM_LAST_COMMAND))
