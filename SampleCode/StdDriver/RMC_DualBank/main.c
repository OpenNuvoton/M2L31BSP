 ...
 * $Revision: 1 $
 * $Date: 23/02/24 4:40p $
 * @brief    Show RMC dual bank capability. Non-blocking program APROM bank1
...
#define DB_PROG_LEN        (4 * RMC_FLASH_PAGE_SIZE) /* Background program length                */
...
enum
{
    ...
    DB_STATE_CLEAR,                                  /* Executing ISP Clear                      */
    DB_STATE_LOAD,                                   /* Executing ISP Load                       */
...
void SysTick_Handler(void)
{
    ...
    if(RMC->ISPSTS & RMC_ISPSTS_ISPBUSY_Msk)
        return;
    ...
    case DB_STATE_START:
        ...
        /* Next state is to program flash */
        g_i8DbState = DB_STATE_CLEAR;
        break;
    case DB_STATE_CLEAR:
        RMC->ISPCMD = RMC_ISPCMD_CLEAR_DATA_BUFFER;  /* ISP word program command                 */
        RMC->ISPADDR = 0x00000000;                   /* word program address                     */
        RMC->ISPTRG = RMC_ISPTRG_ISPGO_Msk;          /* trigger ISP program and no wait          */
        g_i8DbState = DB_STATE_LOAD;                 /* Next state is to Load flash              */
        while(RMC->ISPTRG & RMC_ISPTRG_ISPGO_Msk);
        break;
    case DB_STATE_LOAD:
        RMC->ISPCMD = RMC_ISPCMD_LOAD_DATA_BUFFER;
        RMC->ISPADDR = g_u32DbAddr;                  /* word program address                     */
        RMC->ISPDAT = g_u32DbAddr;                   /* 32-bits data to be programmed            */
        RMC->ISPTRG = RMC_ISPTRG_ISPGO_Msk;          /* trigger ISP program and no wait          */
    ...
    case DB_STATE_PROGRAM:
        RMC->ISPCMD = RMC_ISPCMD_PROGRAM;            /* ISP word program command                 */
        RMC->ISPADDR = g_u32DbAddr;                  /* word program address                     */
        RMC->ISPDAT = g_u32DbAddr;                   /* 32-bits data to be programmed            */
        RMC->ISPTRG = RMC_ISPTRG_ISPGO_Msk;          /* trigger ISP program and no wait          */
        g_u32DbAddr += 4;                            /* advance to next word                     */
        g_u32DbLength -= 4;
        g_i8DbState = DB_STATE_CLEAR;                /* Next state is to clear flash             */
        break;
...
int32_t main(void)
{
    ...
    printf("+------------------------------------------+\n");
    printf("|         RMC Dual Bank Sample Demo        |\n");
    printf("+------------------------------------------+\n");
