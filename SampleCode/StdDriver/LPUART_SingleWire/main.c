...
void SYS_Init(void)
{
    ...
    /* Select LPUART0 clock source is HIRC */
...
/*---------------------------------------------------------------------------------------------------------*/
/* LPUART0 Callback function                                                                                  */
/*---------------------------------------------------------------------------------------------------------*/
void LPUART0_TEST_HANDLE()
{

    if (LPUART_GET_INT_FLAG(LPUART0,LPUART_INTSTS_RDAIF_Msk))
    {
        /* Get all the input characters */
        while (LPUART_IS_RX_READY(LPUART0))
        {
            /* Get the character from LPUART Buffer */
            g_u8RecData[g_u32RecLen] = LPUART_READ(LPUART0);
    ...
    if (LPUART_GET_INT_FLAG(LPUART0,LPUART_INTSTS_SWBEIF_Msk))
    {
        printf("Single-wire Bit Error Detection \n");
        LPUART_ClearIntFlag(LPUART0, LPUART_INTSTS_SWBEINT_Msk);
...
void UART_FunctionTest()
{
    ...
    printf("+-----------------------------------------------------------+\n");
    printf("|            UART/LPUART Single Wire Function Test          |\n");
    printf("+-----------------------------------------------------------+\n");
    printf("|  Description :                                            |\n");
    ...
    do
    {
        ...
        case '2':
        {
            ...
            /* Check the Rx status is Idel */
            while (!LPUART_RX_IDLE(LPUART0)) {};

            LPUART_Write(LPUART0, g_u8TxData, BUFSIZE);
