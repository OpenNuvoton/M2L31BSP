...
int32_t main(void)
{
    ...
    printf("input is between the range of VDDA * 5 / 63 and bandgap.\n");
    printf("Press any key to continue ...");
    getchar();
    printf("\n");

    /* Select VDDA as CRV0 source */
    ACMP_SELECT_CRV0_SRC(ACMP01, ACMP_VREF_CRV0SSEL_VDDA);
    /* Select CRV0 level: VDDA * 5 / 63 */
    ACMP_CRV0_SEL(ACMP01, 5);
    /* Configure ACMP0. Enable ACMP0 and select CRV0 as the source of ACMP negative input. */
