 ...
 * @version  V1.00
 * @brief    M2L31 Series Data Flash Access API
 *
...
void RMC_ISP(uint32_t u32Cmd, uint32_t u32Addr, uint32_t u32Data)
{
...
#if 0
    RMC_ISP(RMC_ISPCMD_READ_DID, 0, 0);

    u32ROMSEL = (RMC->ISPDAT >> 4) & 0x0F;
...
void DataFlashProgramPage(uint32_t u32StartAddr, uint32_t *u32Buf)
{
    ...
    {
        RMC_ISP(RMC_ISPCMD_PROGRAM, u32StartAddr + i * 4, u32Buf[i]);
    }
}
