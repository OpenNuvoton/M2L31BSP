...
__ALIGNED(4) static uint8_t aprom_buf[RMC_FLASH_PAGE_SIZE];
...
static uint16_t CalCheckSum(uint32_t start, uint32_t len)
{
    ...
    for (i = 0; i < len; i += RMC_FLASH_PAGE_SIZE)
    {
        ReadData(start + i, start + i + RMC_FLASH_PAGE_SIZE, (uint32_t *)aprom_buf);

        if (len - i >= RMC_FLASH_PAGE_SIZE)
        {
            lcksum += Checksum(aprom_buf, RMC_FLASH_PAGE_SIZE);
        }
        else
        {
